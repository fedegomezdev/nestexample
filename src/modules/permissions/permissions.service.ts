...
export class PermissionsService {
  ...
  create = async (data: CreatePermissionInput): Promise<string> => {
    try {
      const exist = await this.permissionRepository.find({ where: { aplication: data.aplication, ecommerce: data.ecommerce } });
      if (exist.length > 0) throw new Error('This ecommerce already has the permission');
